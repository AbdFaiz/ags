...
<nav id="sidebarMenu" class="sidebar d-lg-block bg-gray-800 text-white collapse" data-simplebar>
    <div class="sidebar-inner px-2 pt-3">
        ...
        <ul class="nav flex-column pt-3 pt-md-0">
            ...
            <li class="nav-item">
                <span class="nav-link {{ Request::segment(1) == 'emails' ? '' : 'collapsed' }} d-flex justify-content-between align-items-center"
                    data-bs-toggle="collapse" data-bs-target="#submenu-mail" aria-expanded="{{ Request::segment(1) == 'emails' ? 'true' : 'false' }}">
                    <span>
               ...
               <div class="multi-level collapse {{ Request::segment(1) == 'emails' ? 'show' : '' }}" role="list" id="submenu-mail" aria-expanded="false">
                    <ul class="flex-column nav">
                        {{-- Folder Utama (Inbox, Starred, Sent, Trash) --}}
                        @foreach ([
                            'INBOX' => ['icon' => 'inbox', 'label' => 'Inbox'],
                            'STARRED' => ['icon' => 'star', 'label' => 'Starred'],
                            'SENT' => ['icon' => 'paper-plane', 'label' => 'Sent'],
                            'TRASH' => ['icon' => 'trash', 'label' => 'Trash'],
                        ] as $folderKey => $data)
                            @php
                                // Logic penentuan URL dan Active State
                                if ($folderKey === 'STARRED') {
                                    $url = route('email.index', ['flagged' => 1]);
                                    $isActive = request('flagged') == 1;
                                } else {
                                    $url = route('email.index', ['folder' => $folderKey]);
                                    $isActive = (request('folder') === $folderKey || ($folderKey === 'INBOX' && !request('folder') && !request('label') && !request('flagged') && Request::segment(1) == 'emails'));
                                }
                            @endphp

                            <li class="nav-item {{ $isActive ? 'active' : '' }}">
                                <a href="{{ $url }}" class="nav-link d-flex justify-content-between">
                                    <span>
                                        <span class="sidebar-icon-child me-2">
                                            <i class="fas fa-{{ $data['icon'] }} fa-xs"></i>
                                        </span>
                                        <span class="sidebar-text">{{ $data['label'] }}</span>
                                    </span>
                                </a>
                            </li>
                        @endforeach
                        ...
                        {{-- Divider Label --}}
                        @if(isset($availableLabels) && $availableLabels->count() > 0)
                            <li class="nav-item">
                                <hr class="my-2 border-secondary opacity-25 mx-3">
                                <small class="text-uppercase px-4 fw-bold text-muted" style="font-size: 10px; letter-spacing: 1px;">Labels</small>
                            </li>

                            {{-- Loop Label Dinamis --}}
                            @foreach ($availableLabels as $labelName)
                                <li class="nav-item {{ request('label') === $labelName ? 'active' : '' }}">
                                    <a href="{{ route('email.index', ['label' => $labelName]) }}" class="nav-link d-flex justify-content-between">
                                        <span>
                                            <span class="sidebar-icon-child me-2"><i class="fas fa-tag fa-xs text-info"></i></span>
                                            <span class="sidebar-text">{{ ucfirst($labelName) }}</span>
                                        </span>
                                    </a>
                                </li>
                            @endforeach
           ...
           <!--  <li class="nav-item {{ Request::segment(1) == 'transactions' ? 'active' : '' }}">
            ...
            <li class="nav-item">
                <a href="#" target="_blank" class="nav-link d-flex align-items-center">
                    ...
                    <span class="sidebar-text">CMS Web</span>
                </a>
            </li>
        </ul>
    </div>
